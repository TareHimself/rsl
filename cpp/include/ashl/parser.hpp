...
namespace ashl {
    ...
    // void getTokensTill(TokenList& result,TokenList& input,const std::set<ETokenType>& ends,int initialScope = 0,bool popEnd = true);
    // std::shared_ptr<ScopeNode> parseScope(TokenList& input);


    TokenList consumeTokensTill(TokenList& input,const std::set<ETokenType>& targets,const int& initialScope = 0,const bool& includeTarget = false);
